...
import { CheckCircle, CloseRounded, EventAvailable, LocationOn, Phone } from '@mui/icons-material';
...
function ConfirmOrder(props) {
  ...
  const [isFinalPrice, setIsFinalPrice] = useState(true);
  ...
  useEffect(() => {
    if (props.typeValue || props.typeValue === 0) {
    ...
    if (props.cart.length) {
      ...
      if (props.amount) {
        if (props.isFirstOrderDiscount) {
          setDiscountedAmount(
            props.amount -
              ((props.amount * firstOrderDiscount) / 100 > maximumDiscount
                ? maximumDiscount
                : (props.amount * firstOrderDiscount) / 100)
          );
      ...
      if (payment === 'card') {
        ...
        if (order) {
          ...
          try {
            ...
            if (orderObj) {
              props.orderConfirmed(orderObj.id);
              setLoading(false);
            } else {
              setLoading(false);
            }
          } catch (err) {
            if (err.message === 'Error: Invalid Token' || err.message === 'Error: Invalid Session') {
              handleLogout();
            }
            console.error(err);
            setLoading(false);
  ...
  return (
    <>
      ...
      {paymentModel ? (
        <PayOrder {...props} delivery={delivery} setOrder={setOrder} setPaymentModel={setPaymentModel} />
      ) : (
        <Box
          className='comfirmModal'
          sx={{
            ...modalStyle,
            ...{
              width: { xs: '92.5%', sm: typeValue === 0 ? '90%' : '65%', md: typeValue === 0 ? '60%' : '30%' },
            },
          }}
        >
          <IconButton
          ...
          <Box
            sx={{
              px: { xs: 2, sm: 4 },
              maxHeight: { xs: '80vh', sm: '88vh', md: '88vh' },
              overflowX: 'hidden',
              overflowY: 'auto',
            }}
          >
            <Grid container alignItems='flex-start' justifyContent='center' spacing={1} sx={{ pb: 1 }}>
              ...
              >
                {typeValue === 0 && (
                  <Box
                    sx={{
                      minHeight: { xs: 'auto', sm: '25vh', md: '30vh' },
                    }}
                  >
                    <Typography
                      sx={{ my: { xs: 1, sm: 1 }, color: 'text.primary', textAlign: 'start' }}
                      variant='title'
                      component='p'
                    >
                      Delivery
                    ...
                    {delivery === 'standard' ? (
                      <Typography
                        ...
                        variant='subtitle2'
                      >
                        {discountedAmount < deliveryOfferAmount
                          ? `Spend $${deliveryOfferAmount} and unlock our free Hygienic & Expertly Folded Delivery.`
                          : `Free Hygienic & Expertly Folded Delivery Worth $${standardDelivery}! Since your order is above $${deliveryOfferAmount}.`}
                      </Typography>
                    ) : (
                      <Typography
                        sx={{ mt: 0.5, color: 'text.secondary', marginRight: 2, textAlign: 'left' }}
                      ...
                      >
                        {discountedAmount < deliveryOfferAmount
                          ? `Need It Fast? Spend $${deliveryOfferAmount}+ and Unlock Express Delivery for ONLY $${expressDeliveryOffer}! (Valued at $${expressDelivery})`
                          : `Your Order Qualifies! Express Delivery at DISCOUNTED PRICE — Only $${expressDeliveryOffer}!`}
                      </Typography>
                    )}
                  </Box>
                )}
                  ...
                  <>
                    ...
                    >
                      {props.services.map((item, index) => (
                        <>
                          {props.selectedServices.indexOf(item.id) > -1 && (
                            <Grid
                              key={index}
                              container
                              alignItems='center'
                              ...
                              spacing={1}
                              sx={{ borderBottomWidth: 1, py: { xs: 0.5, sm: 0.5 } }}
                            >
                              <Grid sx={{ textAlign: 'start' }} item xs={7.5} sm={7.5} md={7.5}>
                                <Typography sx={{ color: 'text.secondary' }} variant='title' component='h6'>
                                  {item.name}
                                </Typography>
                              </Grid>
                              {item.currentPrice && (
                                <Grid item xs={4.5} sm={4.5} md={4.5}>
                                  ...
                                  >
                                    <Typography sx={{ color: 'text.secondary', pr: 1 }} variant='title' component='h6'>
                                      $ {item.currentPrice}
                                      {item.isItem && ' / lbs'}
                                    </Typography>
                                  </Box>
                ...
                <Stack
                  direction='row'
                  spacing={0.5}
                  sx={{ mt: { xs: 1, sm: 1, md: 1 }, mb: { xs: 0.5, sm: 0.5, md: 1.5 } }}
                  useFlexGap
                ...
                >
                  ...
                  <Typography
                    item
                    sx={{ color: 'text.secondary', display: 'inline', textAlign: 'start', fontWeight: 'bold' }}
                    variant='subtitle1'
                    component='p'
                  >
                ...
                <Stack direction='row' spacing={0.5} useFlexGap justifyContent='start' alignItems='start'>
                  ...
                  <Box>
                    <Typography
                      sx={{ color: 'text.secondary', textAlign: 'start', fontWeight: 'bold' }}
                      component='p'
                      variant='subtitle1'
                    >
                      {props.address.name}
                    ...
                    <Typography sx={{ color: 'text.secondary', textAlign: 'start' }} variant='subtitle1'>
                      <Phone sx={{ fontSize: 16 }} /> {formatMobile(props.address.phone)}
                    </Typography>
                  </Box>
                </Stack>
              </Grid>
              {typeValue === 0 && (
                <Grid item xs={12} sm={6} md={6} alignItems='flex-start' justifyContent='center' spacing={1} sx={{ mt: 1 }}>
                  ...
                  >
                          ...
                          <svg xmlns='http://www.w3.org/2000/svg' width='40' height='40' viewBox='0 0 32 32'>
                            <g className='nc-icon-wrapper'>
                              ...
                              <path
                                ...
                                strokeWidth='0'
                              ></path>
                              <path fill='#231f20' strokeWidth='0' d='M7.448 14.116H8.185V17.893H7.448z'></path>
                              <path
                    ...
                    {payment === 'cash' && (
                      <>
                        ...
                        <Typography
                          component='p'
                          sx={{ mb: 0.5, color: 'text.secondary', textAlign: 'start' }}
                          variant='subtitle2'
                        >
                          Pay at pickup or delivery with cash or card.
                        </Typography>
                      </>
                    )}
                  </Box>
                  ...
                  <Box
                    sx={{
                      maxHeight: { xs: 'auto', sm: '27vh', md: '27vh', lg: '32vh' },
                      overflowX: 'hidden',
                      overflowY: 'auto',
                    }}
                  >
                    {props.cart.map((item, index) => (
                      <Box key={index} sx={{ borderBottomWidth: 1, py: { xs: 0.5, sm: 0.5 } }}>
                        <Grid container alignItems='center' justifyContent='center' spacing={1}>
                          <Grid sx={{ textAlign: 'start' }} item xs={9} sm={9} md={9}>
                    ...
                    {props.isFirstOrderDiscount && (
                      ...
                      >
                        <Grid sx={{ textAlign: 'start' }} item xs={9} sm={9} md={9}>
                          <Typography sx={{ color: 'text.secondary' }} variant='subtitle2' component='p'>
                            <Typography
                              sx={{
                    ...
                    <Grid
                      ...
                      justifyContent='center'
                      spacing={1}
                      sx={{ borderBottomWidth: 1, py: { xs: 0.5, sm: 0.5 }, borderColor: 'text.secondary' }}
                    >
                      <Grid sx={{ textAlign: 'start' }} item xs={9} sm={9} md={9}>
                      ...
                      <Grid item xs={3} sm={3} md={3}>
                        ...
                        >
                          <Typography
                            sx={{ color: 'text.secondary', fontWeight: 'bold', pr: 1 }}
                            variant='subtitle2'
                            component='h6'
                          >
                    ...
                    {(discountedAmount < deliveryOfferAmount || delivery === 'express') && (
                      <Grid
                        container
                        alignItems='center'
                        justifyContent='center'
                        spacing={1}
                        sx={{ borderBottomWidth: 0.75, py: { xs: 0.5, sm: 0.5 } }}
                      >
                        <Grid sx={{ textAlign: 'start' }} item xs={9} sm={9} md={9}>
                          <Typography sx={{ color: 'text.secondary' }} variant='subtitle2' component='h6'>
                            {delivery === 'express' ? 'Express Delivery and Handling' : 'Standard Delivery and Handling'}
                          </Typography>
                        </Grid>
                        <Grid item xs={3} sm={3} md={3}>
                          <Box
                    ...
                    {discountedAmount >= deliveryOfferAmount &&
                      discountedAmount < handlingOfferAmount &&
                      delivery !== 'express' && (
                        <Grid
                          container
                          alignItems='center'
            ...
            {typeValue === 1 && (
              <Typography sx={{ color: 'error.main' }} variant='subtitle1' component='p'>
                <b>Note:</b> The minimum order amount is ${minimumOrderAmount}.
              </Typography>
            )}
            <Button onClick={create} sx={{ mt: { xs: 2, sm: 2 }, px: 4, mb: 1 }} variant='contained'>
              {
                <>
                  {typeValue === 0 ? (
                    ...
                    <Typography variant='title'>Confirm Pickup</Typography>
                  )}
                </>
              }
            </Button>
