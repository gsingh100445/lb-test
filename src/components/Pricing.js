...
export default function Pricing(props) {
  ...
  React.useEffect(() => {
    document.title = 'Pricing - Laundry Basket, Laundry & Dry Cleaning, Pickup and Delivery!';
  }, []);
        ...
        >
          ...
          <div style={{ zIndex: 99 }} className='text-center'>
            <div className='text-center'>
              <Typography className='text-center' sx={{ color: 'text.white', typography: { sm: 'h4', xs: 'h6' } }}>
                HASSLE-FREE LAUNDRY AT AFFORDABLE PRICING
              </Typography>
            </div>
            <Button sx={{ mt: { xs: 2, sm: 3 }, fontSize: 18, fontWeight: 600, px: 3 }} variant='contained'>
          ...
          <Grid container spacing={1} sx={{ pt: 2, pb: 4, px: { xs: 2, sm: 5 } }}>
            <Grid sx={{ display: { xs: 'flex', sm: 'none' } }} container spacing={1} item xs={12}>
              {services.map((serviceItem, itemIndex) => (
                <Grid key={itemIndex} sx={{ mt: 2 }} item xs={6} sm={6} md={6}>
                  <Box
                    sx={{
                      borderWidth: '1px',
                  ...
                  {services[index]?.isItem ? (
                    <>
                      ...
                      >
                        ...
                        <Button sx={{ mt: { xs: 2, sm: 2 }, fontSize: 15, fontWeight: 600, px: 1 }} variant='contained'>
                          <Link to={props.isLoggedIn ? '/book-order' : '/login?redirect=book-order'}>Schedule PickUp</Link>
                        </Button>
                      </Box>
                    </>
