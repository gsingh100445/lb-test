...
const translateObj = {
  ...
  tooManyOTPError: 'Too many OTP requests. Please try again after 15 minutes.',
};

const handleError = (error, functionName) => {
  if (error.response && error.response.data && error.response.data.error) {
    console.error(functionName, error.response.data.error);
...
const handleErrorResp = (response, functionName) => {
  if (response.data && response.data.error) {
    console.error(functionName, response.data.error);
    throw new Error(response.data.error);
  } else {
    console.error(functionName, response);
    throw new Error(translateObj.somethingWrongError);
  }
};
...
export const getError = (error) => {
  ...
  switch (error) {
    ...
    case 'TOO_MANY_REQUESTS':
      msg = translateObj.tooManyOTPError;
      break;
    default:
...
export const getUser = async (id) => {
  try {
    ...
    if (response.data && response.data.isSuccess) {
      ...
      return handleErrorResp(response, 'getUser');
    }
  } catch (error) {
...
export const validateDiscount = async (code) => {
  try {
    let config = await getTokenConfig();
    const response = await axios.get(URL + 'users/validateDiscount/' + code, config);
    if (response.data && response.data.isSuccess) {
      return response.data.data;
    } else {
      console.error(response.data.error);
...
export const getBlogs = async (query) => {
  try {
    const response = await axios.get(URL + 'blogs', { ...config, query });
    if (response.data && response.data.isSuccess) {
      return response.data.page;
    } else {
      console.error(response.data.error);
...
export const getBlog = async (id) => {
  try {
    const response = await axios.get(URL + 'blogs/' + id, config);
    if (response.data && response.data.isSuccess) {
      return response.data.data;
    } else {
      console.error(response.data.error);
